...
export const ApartmentCard = ({ data, sortType }) => {
  const {
    id,
    active,
    ageRange,
  ...
  return (
    <div className="relative">
      <article
        ...
        <figure className={`flex-none relative ${sortType === 'GRID' ? 'h-44 lg:h-52' : 'h-72'} transition-all w-full rounded-sm box-border`}>
          <img
            src={apartmentRequest?.propertyImages[0]}
          ...
          <div className='absolute lg:hidden bg-blue-600 text-xs text-white flex items-center left-3 rounded-sm p-2 h-8 shadow-md top-4'>
            {/* {matchingPercentage + '% '} */}
            100% match
          </div>
        ...
        <div className="flex-auto flex flex-col gap-2.5 lg:gap-3 text-xs py-1.5 pb-2.5 px-1.5 hover:opacity-90 active:opacity-100">

          <div className="flex flex-col w-full">
            <p className="text-sm font-medium">Looking for  {propertyType}{' '}
              {isRoommateRequest ? 'a Student roommate' : 'an apartment'}
            </p>
          ...
          <div className="self-left w-3/4 maxscreen:text-[11px] flex items-center flex-wrap gap-x-3">
            ...
            <p className="flex items-center gap-1.5 text-gray-700">
              ...
              <span>
                {apartmentRequest?.propertySummary?.bedroomsNumbers}
                Bedroom</span>
            </p>
            <p className="flex items-center gap-1.5 text-gray-700">
          ...
          <div className="flex items-center w-full justify-between maxscreen:text-[11px] text-sm">
            ...
            <div className='flex items-center gap-1'>
              ...
              <figure className='w-8 h-8 mobile:w-10 mobile:h-10 box-border rounded-full'>
                <img
                  src={user?.imageUrl}
                  alt='Home' loading='eager'
              ...
              <div className='flex flex-col -gap-1 font-medium mobile:text-base'>
                <p
                  ...
                  <b>
                    {reduceTextLength(user?.firstName, 15, 'letter')}
                    , 27</b>
            ...
            <p className='text-black text-xs'>
              {
                apartmentRequest?.price?.priceType === 'MONTHLY'
                  ? (
                    apartmentRequest?.price?.amount > 0
                      ? `#${formatPrice(apartmentRequest?.price?.amount)}/monthly` : 'NA'
                  )
                  :
                  (
                    apartmentRequest?.price?.amount > 0
                      ? `#${formatPrice(apartmentRequest?.price?.amount)}/daily` : 'NA'
                  )
              }
            </p>
